...
from .formats import FORMAT_REGISTRY, get_format, get_kv_format
...
def _resolve_quantizer_spec(spec: QuantizerSpec, target: str) -> dict[str, Any] | list[dict]:
    ...
    """
    if spec.stages:
        return [_resolve_quantizer_spec(stage, target) for stage in spec.stages]
...
def _apply_override(quant_cfg: dict, override: OverrideEntry) -> None:
    ...
    if override.pattern:
        ...
        if override.activations:
            mc_cfg["*input_quantizer"] = _resolve_quantizer_spec(
                override.activations, "activation"
            )
...
def _resolve_auto_quantize(section: AutoQuantizeSection) -> dict[str, Any]:
    ...
    format_configs = []
    for fmt_entry in section.formats:
        format_configs.append(get_preset(fmt_entry.preset))
